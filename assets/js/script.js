let designcl = ['Work','Home','Personal','Others']; //creating class for implementating design to different category
$(document).ready(function(){

    let categorys = document.getElementsByClassName('catesec'); //getting all the class name category
    for(let i=0; i<categorys.length;i++){ //looping in the categorys to find which category class belongs and implement according session check home.css to get the color of each section
        if(categorys[i].innerHTML.trim() == 'Work'){
            categorys[i].classList.add(designcl[0]);
            categorys[i].classList.add('commonClass');
        }

        else if(categorys[i].innerHTML.trim()=='Home'){
...
function checkedOrNot(){
    let cb = document.querySelectorAll('.delechack');//getting all the checkbox class
    let descdisp = document.querySelectorAll('.dispdsc');//getting all the class where description of todo is defined
    ...
    for(let i=0; i<descdisp.length;i++){
        ...
        //checking if checkbox is checked, if checked a line will pass through the text(-) else if it is unchecked no line will pass through date description
...
document.getElementById('deleteButton').addEventListener('click', function(){
    let checkedValue = document.querySelectorAll('.delechack:checked') //getting only checked value
    ...
    $.ajax({
        ...
        success: function(){ //on ajax success i.e when data is delete
            swal("Item is deleted ", "click ok to go back Home ", "success") //using sweetalert to show that data is deleted
            .then(redir => {
                window.location = '/';
            })
